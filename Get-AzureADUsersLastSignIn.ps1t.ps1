function Connect-AzureDevicelogin {
    ...
    param( 
        ...
        [Parameter()]
    #    $TenantID = 'common',
        $TenantID = '28dfcc89-f68e-4a13-b297-fca7fc334f29',
...
foreach($ADUser in $AADUsers){
    ...
    $ADUserepobj | Add-Member NoteProperty -Name "Created DateTime (UTC)" -Value (get-date($ADUser.createdDateTime) -Format dd/MM/yyyy)
    if (($ADUser.signInActivity).lastSignInDateTime) {
        $ADUserepobj | Add-Member NoteProperty -Name "Last interactive Signin (UTC)" -Value (get-date(($ADUser.signInActivity).lastSignInDateTime) -Format dd/MM/yyyy)}
            else{
                $ADUserepobj | Add-Member NoteProperty -Name "Last interactive Signin (UTC)" -Value ""}
    if (($ADUser.signInActivity).LastNonInteractiveSignInDateTime) {
        $ADUserepobj | Add-Member NoteProperty -Name "Last Success Non-interactive Signin (UTC)" -Value (get-date(($ADUser.signInActivity).LastNonInteractiveSignInDateTime) -Format dd/MM/yyyy)}
            else{$ADUserepobj | Add-Member NoteProperty -Name "Last Success Non-interactive Signin (UTC)" -Value ""}
